...
## Usage:
Spark is a library that lets the user have granular control over the necessary things, but seamlessly handles the rest under the hood.
...
```js
// create an instance of the Spark. This will be the collection of its ParticleGroups and Particles. You can have multiple of these running simultaneously, ex. one for directional combat bursts and one for generic firework-like bursts
...
// this creates a ParticleGroup. It can be accessed through the Spark through its [.activeParticleGroups] property. Once a ParticleGroup has been created, its lifespan is 300ms. It should be handled in the draw() loop immediately.
...
// direction (optional). This can make the particles burst in one direction, not the firework-like default of {x: 0, y: 0}. These values are added each update to the pos object. Negative values are accepted
const direction = {
    x: 0,
    y: 1
}
...
// Loop through all Particle instances and draw them
...
// let myOpacity: number = myParticle.getOpacity returns the current opacity
